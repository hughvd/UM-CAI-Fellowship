...
"""
    ...
    def find_similar_courses(self, filtered_df: pd.DataFrame, example_embedding: List[float], top_n: int = 50) -> List[int]: 
        # Calculate similarities for all courses
        similarities = []
        for idx, row in filtered_df.iterrows():
            similarity = self.similarity_calculator.calculate(example_embedding, row['embedding'])
            similarities.append((similarity, idx))
        
        # Find top n indices without sorting the final result
        top_n_indices = heapq.nlargest(top_n, similarities, key=lambda x: x[0])
        top_similarities = [sim for sim, _ in top_n_indices]
        top_indices = [idx for _, idx in top_n_indices]
        
        # Create unsorted version with just the top n courses in original order
        unsorted_df = filtered_df.iloc[top_indices].copy()
        
        # Create sorted version with similarity scores and ranks
        sorted_indices = sorted(range(len(top_similarities)), key=lambda k: top_similarities[k], reverse=True)
        sorted_df = unsorted_df.iloc[sorted_indices].copy()
        sorted_df['similarity_score'] = [top_similarities[i] for i in sorted_indices]
        sorted_df['similarity_rank'] = range(len(sorted_df))
        
        return unsorted_df, sorted_df
    ...
    async def recommend(self, query: str, levels: Optional[List[int]] = None) -> str:
        try:
            ...
            # Generate example description and its embedding (do not sort yet)
            ...
            filtered_df, sorted_df = self.find_similar_courses(filtered_df, example_embedding)
...
# and personal interest in your selections. Do not recommend courses that are not under available courses."""
            ...
            return recommendation, sorted_df
        except Exception as e:
